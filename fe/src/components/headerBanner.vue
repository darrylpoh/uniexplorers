...
<template>
  <div :class="mq.mdPlus ? 'px-16' : (mq.smPlus ? 'px-8' : '')" class="head sticky w-screen bg-darkgreen h-20 flex items-center justify-around border-0 border-b-2 border-gray-500">
    ...
    <div v-if="mq.smPlus" :class="mq.smPlus ? 'w-1/3' : 'w-1/2'" class="searchBar relative h-1/2">
      <!-- BUG: textbox not aligned -->
      <input type="text" class="relative rounded-xl w-full h-full pl-4 text-black" placeholder="University?" v-model="search" >
      <div class="searchIcon w-auto h-full px-2 absolute flex items-center right-0 top-0 mt-1 cursor-pointer hover:scale-110">
