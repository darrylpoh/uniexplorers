            ...
            updateVote(icon) { 
                if (icon === 'like') {
                    ...
                    // Call the backend API here
                        if (this.upvoteFill || this.downvoteFill) {
                            console.log('backend was called to ' + icon);
                        }
                        // axios.get(import.meta.env.VITE_BACKEND + `/forum/comment/${icon}/${this.commentData.id}`).then(res => {
